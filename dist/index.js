...
function createRouter() {
  ...
  function insert(node2, path, method, handlers) {
    if (path === "") {
      const map = node2.handlers ??= {};
    ...
    if (path[0] === "/" && path[1] === ":" || path[0] === ":") {
      ...
      node2.paramChild ??= { prefix: "", children: [], paramName };
      const rest2 = nextSlash === -1 ? "" : path.slice(nextSlash);
      insert(node2.paramChild, rest2, method, handlers);
      return;
    }
    if (path[0] === "*") {
      node2.wildcardChild ??= { prefix: "", children: [] };
      const map = node2.wildcardChild.handlers ??= {};
      map[method] = handlers;
      return;
    }
    let child = node2.children.find((c) => path.startsWith(c.prefix));
    if (!child) {
  ...
  async function dispatch(ctx, next) {
    ...
    while (i < pathname.length) {
      const child = node2.children.find((c) => pathname.startsWith(c.prefix, i));
      if (child) {
      ...
      if (node2.paramChild) {
        ...
        params[node2.paramChild.paramName] = decodeURIComponent(pathname.slice(i + 1, end));
        i = end;
    ...
    const handlers = (done ? node2.handlers && node2.handlers[ctx.method] : undefined) || node2.wildcardChild?.handlers && node2.wildcardChild.handlers[ctx.method];
    if (handlers) {
      let idx = -1;
      const run = async () => {
        idx++;
...
class App {
  ...
  listen(adapter, port = 3000) {
    console.log(import_chalk.default.green(`[${adapter.name}]`), import_chalk.default.cyan(`listening on ${port}`));
    const fastMap = this.fastRoutes;
    if (adapter.name === "bun") {
      adapter.listen(port, async (req) => {
      ...
      adapter.listen(port, async (req, res) => {
        const key = req.method + " " + req.url;
        const fastHandler = fastMap.get(key);
        if (fastHandler) {
          const resp = fastHandler();
          if (resp && "body" in resp && Buffer.isBuffer(resp.body)) {
            const entry = resp;
            res.writeHead(entry.status, headersToObject2(entry.headers));
            res.end(entry.body);
            return new Response(null, { status: entry.status, headers: entry.headers });
          }
          if (resp instanceof Response) {
            res.writeHead(resp.status, headersToObject2(resp.headers));
...
var METHOD_COLORS = {
  GET: import_chalk2.default.green,
  POST: import_chalk2.default.blue,
  PUT: import_chalk2.default.yellow,
  DELETE: import_chalk2.default.red,
  PATCH: import_chalk2.default.magenta,
  OPTIONS: import_chalk2.default.cyan
};
var TIME_COLORS = {
  slow: import_chalk2.default.red,
  medium: import_chalk2.default.yellow,
  fast: import_chalk2.default.green
};
var logger = () => async (ctx, next) => {
  const t = performance.now();
  const result = await next();
  const time = Math.round(performance.now() - t);
  const colorFn = METHOD_COLORS[ctx.method] || import_chalk2.default.white;
...
export {
  exports_plugins as plugins,
  nodeAdapter,
  logger,
  createRouter,
  exports_core as core,
  bunAdapter,
  exports_adapters as adapters,
  TIME_COLORS,
  METHOD_COLORS,
  App
};
